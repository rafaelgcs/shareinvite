        ...
        <div className="min-h-screen bg-[#F9F8F6]">
                    ...
                    <div className="flex h-14 justify-between items-center">
                        <div className="flex items-center gap-10">
                            <div className="flex shrink-0 items-center">
                                <Link href="/" className="flex items-center gap-3 group">
                                    ...
                                    <span className={`font-serif text-2xl tracking-tighter transition-colors duration-500 hidden sm:block ${scrolled ? 'text-[#0A0A0A]' : 'text-white'
                                        }`}>
                            ...
                            <div className="hidden sm:flex items-center gap-1">
                                <Link
                                    ...
                                    className={`relative px-6 py-2 text-sm font-black uppercase tracking-[0.15em] transition-all duration-300 group ${route().current('dashboard')
                                ...
                                <button
                                    className={`relative px-6 py-2 text-sm font-black uppercase tracking-[0.15em] transition-all duration-300 group ${scrolled ? 'text-stone-400 hover:text-[#0A0A0A]' : 'text-stone-500 hover:text-white'
                                        }`}
                                >
                                    <span className="relative z-10 flex items-center gap-2">
                            ...
                            <div className="relative">
                                <Dropdown>
                                    <Dropdown.Trigger>
                                        <button
                                            type="button"
                                            className={`inline-flex items-center gap-3 rounded-[1.25rem] border px-4 py-2 text-sm font-bold transition-all duration-500 group ${scrolled
                        ...
                        <div className="-me-2 flex items-center sm:hidden">
                            <button
                                ...
                                className={`inline-flex items-center justify-center rounded-2xl p-2.5 transition-all duration-300 ${scrolled
                ...
                <AnimatePresence>
                    {showingNavigationDropdown && (
                        <motion.div
                            ...
                            exit={{ opacity: 0, y: -10 }}
                            className={`sm:hidden overflow-hidden border-t shadow-2xl ${scrolled ? 'bg-white border-stone-100' : 'bg-[#0A0A0A] border-white/5'
                                }`}
                        >
                            <div className="space-y-2 px-4 pb-8 pt-6">
                                <ResponsiveNavLink
                                    ...
                                    className={`flex items-center gap-4 px-5 py-4 rounded-[1.5rem] text-base font-black transition-all ${route().current('dashboard')
                                ...
                                <div className={`mt-8 pt-8 border-t ${scrolled ? 'border-stone-100' : 'border-white/5'}`}>
                                    ...
                                    <ResponsiveNavLink
                                        ...
                                        className={`flex items-center gap-4 px-5 py-4 rounded-2xl text-base font-bold ${scrolled ? 'text-stone-600 hover:bg-stone-50' : 'text-stone-400 hover:bg-white/5 hover:text-white'
                                            }`}
                                    >
            ...
            <footer className="py-12 border-t border-stone-200 mt-20">
                <div className="max-w-7xl mx-auto px-4 text-center">
                    <p className="text-stone-400 text-sm font-medium">© 2026 Miu Invites — Seu convite inesquecível.</p>
                </div>
            </footer>
        </div>
    );
